...
| `describe_sheet(spreadsheet_id, account?)` | ~500ms | function | `backends.sheets` (gws-as.sh) |
| `read_cells(spreadsheet_id, range, account?)` | ~500ms | function | `backends.sheets` |
| `write_cells(spreadsheet_id, edits[], account?)` | ~600ms | function | `backends.sheets` (values.batchUpdate; batches N edits in one call) |
| `drive_search(query, account?, limit)` | 500–1500ms | function | `backends.gws` (fan-out across accounts) |
...
Per-tool TTL cache in `server.py:_TOOL_TTL` keyed on `(tool, args_hash)` via `backends/cache.py`. Tools in `server.py:_NO_CACHE` (`read_cells`, `write_cells`) bypass the cache entirely — a mutating write must re-execute when re-issued, and a live read used to verify a write must reflect the just-written state (the 300s default TTL would mask both). The Sheets tools are the direct, sub-second path for spreadsheet work; `_BASE_PROMPT` rule 2b routes sheet reads/edits here instead of `deep_research`.
...
- `dossier.refresh_dossier()`: one agent call with a JSON-only directive → atomic write to `DOSSIER_PATH`. 30-min debounce; `force=True` bypasses. Refreshed on server boot (retry-with-backoff to ride out the gateway boot-race, where the mini and the `:8642` gateway start together under launchd), on a periodic timer (`server.py:_dossier_refresh_loop`, `DOSSIER_REFRESH_INTERVAL_SEC` default 40min — self-heals long no-call staleness), and after every call ends. Schema: `{open_loops, calendar_today, recent_decisions, hot_people, last_handoff_summary, working_state_from_last_call}`.
